...
## Что умеет
- Собирает **базовые образы** (ROS2 + CUDA) под три платформы (x86, Jetson AGX Orin, Jetson Orin Nano).
...
| Jetson Orin Nano (JP 7-класс) | arm64 | 87 | `nvidia/cuda:13.1.0-devel-ubuntu24.04` | Jazzy | ✅ |

> Примечание по Orin Nano: официального l4t-образа под JP7 ещё нет, поэтому базой взят
> совместимый CUDA 13 / Ubuntu 24.04 (arm64). JP7 = Ubuntu 24.04 → ROS2 Jazzy. Для сборки
> FAST-LIO2 под Jazzy применён патч стандарта на C++17 (см. `docs/add-package.md`).

## Опубликованные образы (ghcr.io)
Базы:
- `ghcr.io/ilyasalih/ros2-cuda-x86:humble-cuda12.6`
- `ghcr.io/ilyasalih/ros2-cuda-jetson-agx:humble-l4t-r36.4` (нативно) и `…-r36.4-qemu` (кросс QEMU)
- `ghcr.io/ilyasalih/ros2-cuda-jetson-nano:jazzy-cuda13-ubuntu24.04`

Пакет FAST-LIO2:
- `ghcr.io/ilyasalih/fast-lio2-x86:humble-cuda12.6`
- `ghcr.io/ilyasalih/fast-lio2-jetson-agx:humble-l4t-r36.4`
- `ghcr.io/ilyasalih/fast-lio2-jetson-nano:jazzy-cuda13`
...
- [docs/add-package.md](docs/add-package.md) — как добавить новый ROS2-пакет и **как добавить платформу**.
